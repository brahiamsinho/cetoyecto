...
import { useDocenteFormController } from '../controllers/useDocenteFormController'

export default function DocenteFormView({ state, handlers }) {
  const { form, loading, saving, alert, isEdit } = state
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="profesion" value={form.profesion} onChange={handleChange} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required />
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={() => window.history.back()} className="px-6 py-2.5 border border-slate-300 text-slate-700 font-medium rounded-lg text-sm hover:bg-slate-50">
              Cancelar
            </button>
          </div>
        </form>
...
export function DocenteFormWrapper() {
  const props = useDocenteFormController()
  return <DocenteFormView {...props} />
}
